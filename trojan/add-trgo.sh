...
BURIQ () {
    curl -sS https://notabug.org/sakai/tetbot/raw/main/main/skkkk > /root/tmp
...
Name=$(curl -sS https://notabug.org/sakai/tetbot/raw/main/main/skkkk | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://notabug.org/sakai/tetbot/raw/main/main/skkkk | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
linktrgo="trojan-go://${uuidR}@${domain}:${trgo}/?sni=${domain}&type=ws&host=${domain}&path=/aidan&encryption=none#${user}"
clear
